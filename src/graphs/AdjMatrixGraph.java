...
public class AdjMatrixGraph<T> implements IGraph<T> {
    ...
    public AdjMatrixGraph(boolean directed, boolean weighted) {
        ...
        weightsMatrix = new ArrayList<List<Double>>();
        map = new HashMap<>();
    }
    ...
    private int getIndexOf(Vertex<T> v) {
        int index = -1;
    ...
    public void prim(Vertex<T> r) {
        ...
        while (!queue.isEmpty()) {
            ...
            List<Vertex<T>> neigh = getNeighbors(u);
            for (Vertex<T> v : neigh) {
                if (v.getColor() == Vertex.WHITE && getEdgeWeight(u, v) < v.getDistance()) {
                    queue.remove(v);
                    v.setDistance(getEdgeWeight(u, v));
                    queue.add(v);
                    v.setPred(u);
                }
            }
    ...
    public List<Edge<T>> kruskal() { // Adapted from
        ...
        List<Edge<T>> result = new ArrayList<>(); // Tnis will store the resultant MST
